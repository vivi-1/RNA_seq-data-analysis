...
# -*- coding: utf-8 -*-
"""
Created on Tue May 26 14:28:43 2020
...
import numpy
currentDirectory_down="/Volumes/WD1/DEGlist/down_regulate"
...
import xlwt
Name_comparefile=""
Name_comparefile="comparison data_Group11.xls" ## Change dynamicly
...
wb=xlwt.Workbook(currentDirectory_comparison+'/'+'log2FoldChange in B_downs_group11.xls','wrb') ## Change dynamicly
...
for i in range(0,len(highlight_ID)):
    ...
    for j in range(0, read_comparefile_col):
        if read_comparefile.iat[i,j].isnumeric()==True:
            searchfile_value=5+4*(j)
            searchfile_value_str=str(searchfile_value)
            Searchfile_name="G1vsG" + searchfile_value_str +"_DEG_down.xlsx"
            Search_file=pandas.read_excel(currentDirectory_down+"/"+Searchfile_name)
            ...
            ws.write(i+1,j+1,value_to_write)
        print("saved data")              
wb.save('log2FoldChange in B_downs_group11.xls') ## Change dynamicly
print("data saved")
