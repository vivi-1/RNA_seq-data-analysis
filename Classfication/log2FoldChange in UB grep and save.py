...
# -*- coding: utf-8 -*-
"""
Created on Tue May 26 14:07:11 2020
...
# sheetname7: C: 6,10,14,18,22,26,30,,,,6+4*(k-1), k is the timepoint value
...
Name_comparefile=""
Name_comparefile="comparison data_Group9.xls" ## Change dynamicly
...
wb=xlwt.Workbook(currentDirectory_comparison+'/'+'log2FoldChange in B_ups_group9.xls','wrb') ## Change dynamicly
...
for i in range(0,len(highlight_ID)):
    ...
    for j in range(0, read_comparefile_col):
        if read_comparefile.iat[i,j].isnumeric()==True:
            ...
            ws.write(i+1,j+1,value_to_write)
        print("saved data")              
wb.save('log2FoldChange in B_ups_group9.xls') ## Change dynamicly
print("data saved")
